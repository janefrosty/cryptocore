...
def write_binary_file(path, data):
    ...
    """
    try:
        # Sprint 6: Ensure directory exists
...
def read_file_with_nonce(filename):
    """
    Sprint 6: Read file and extract nonce from first 12 bytes for GCM
    Returns: (nonce, ciphertext_and_tag)
    """
    data = read_binary_file(filename)
    if len(data) < 12:
        raise ValueError("Input file is too short to contain nonce")
    
    nonce = data[:12]
    rest = data[12:]
    return nonce, rest

def cleanup_failed_file(filename):
    """
    Sprint 6: Clean up file if operation failed
    Used for GCM authentication failures
    """
    try:
